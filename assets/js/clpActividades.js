...
var main,
  ...
  click,
  activityId = xId.pop();
'actividades.html' == page && 'tenis' == activityId
  ? ((main = gallery[2].tenis),
    (id = 'tenis_gallery'),
    (heading = 'centro de tenis'),
    (click = 'displayData(this.id)'))
  : 'actividades.html' == page && 'padel' == activityId
  ? ((main = gallery[3].padel),
    (id = 'padel_gallery'),
    (heading = 'centro de padel'),
    (click = 'displayData(this.id)'))
  : ((id = 'tenis_gallery'), (heading = 'Recreacion Deportivas'));
var heading =
...
for (var n = 0; n < main.notifications.length; n++)
  activity +=
    ...
    '</p></li>';
(activity += '</ul></div></div>'), (activity += '<ul>');
...
(activity += '</ul>'),
  ...
  (activity +=
    '<div id="' + main.activities[0].story_content_id + '"><figure>'),
  (activity +=
  ...
  (activity +=
    '<img src="./assets/images/icons/close-outlined-red.png" alt="close icon" width="25px" height="25px"/></figure></header>'),
  (activity +=
    '<div id="' + main.activities[1].story_content_id + '"><figure>'),
  (activity +=
    '<img src="./assets/images/galleria/eventos/color/Event3.png" alt="event image" /></figure>'),
  (activity += '<h3>El Evento 3</h3>'),
  (activity +=
